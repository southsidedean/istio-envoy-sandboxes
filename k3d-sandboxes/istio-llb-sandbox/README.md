...
This sandbox demonstrates **Locality Load Balancing (LLB)** with **Istio**, keeping traffic within the same topology zone to reduce latency and improve reliability. The cluster is configured with three zones (central, west, east) and uses the **movies app** to generate cross-zone traffic that can be observed and controlled.
...
Deploy curl test pods (one per zone):
...
Check which backend each curl pod is hitting:

```bash
for curlpod in $(kubectl get pods --no-headers -o custom-columns=NAME:.metadata.name | grep curl) ; do echo ; echo "Logs for $curlpod:" ; echo ; kubectl logs "$curlpod" | grep Movie ; echo ; done
```

To clean up the curl test pods:

```bash
for zone in east central west ; do kubectl delete deploy curl-$zone ; done
```

To test without LLB, remove the mechanisms keeping traffic in-zone (see prior sections for the appropriate commands for your deployment mode), then redeploy the curl pods and check the logs again.

## Summary

This sandbox demonstrates how to configure and verify Locality Load Balancing in both Istio sidecar and ambient modes. Use Kiali and the curl test pods to observe traffic distribution with and without LLB enabled.
