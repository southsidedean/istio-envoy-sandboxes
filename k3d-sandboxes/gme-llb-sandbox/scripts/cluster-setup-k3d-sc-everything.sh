...
do
clustername=$CLUSTER_NAME_PREFIX$cluster
k3d cluster create $clustername -c cluster-k3d/k3d-cluster.yaml --port 90${cluster}:80@loadbalancer --port 94${cluster}:443@loadbalancer --port 95${cluster}:8090@loadbalancer --api-port 0.0.0.0:96${cluster} --verbose --trace
done
